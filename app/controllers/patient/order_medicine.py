...
class OrderMedicine(MethodView, BaseService):
    ...
    def get(self):
        ...
        medicine_query = f"SELECT drug_name as name, drug_id as id, price as cost FROM Drug WHERE is_restricted = 0"
        ...
        return render_template("patient/order_medicine.html", name=name, balance=balance, medicine=medicine, notifications=self.patient_service.fetch_notifications(self.uid), basket=basket_str, total_cost=total_cost)
